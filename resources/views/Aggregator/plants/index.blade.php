...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('aggregator.plants.store') }}" method="POST">
            @csrf
...
<script>
    function editPlant(plant) {
        ...
        document.getElementById('editForm').action = `/aggregator/plants/${plant.id}`; // Set the form action dynamically
        $('#editModal').modal('show'); // Show the modal
    }

    function confirmDelete(plantId) {
        const deleteForm = document.getElementById('deleteForm');
        deleteForm.action = `/aggregator/plants/${plantId}`; // Set the form action dynamically
