...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('user.plants.store') }}" method="POST">
        @csrf
                        ...
                        @foreach($plants as $plant)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-warning btn-sm" onclick="editPlant({{ $plant }})">
                                        <i class="fas fa-edit"></i> 
                                    </button>

                                    <!-- Delete Button to trigger modal -->
                                    <button class="btn btn-danger btn-sm" onclick="confirmDelete({{ $plant->id }})">
...
<script>
    function editPlant(plant) {
        ...
        document.getElementById('editForm').action = `/user/plants/${plant.id}`; // Set the form action dynamically
        $('#editModal').modal('show'); // Show the modal
    }

    function confirmDelete(plantId) {
        const deleteForm = document.getElementById('deleteForm');
        deleteForm.action = `/user/plants/${plantId}`; // Set the form action dynamically
